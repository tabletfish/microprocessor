...
void initTimer2() {
    ...
    OCR2A = 156; // 16MHz / 1024 / 100 ≈ 156 (10ms)
    TIMSK2 = (1 << OCIE2A); 
}
...
void loop() {
    ...
    if (phonSerial.available()) {
        ...
        if (c >= 32 && c <= 126) {
            ...
            if (TextIndex > 0) {
                ...
                if (!NowVibrating && !EmptyTextVibration) {
                    uart_println("[BT RECV] Empty text received, starting vibration");
    ...
    if (VibrationCompleteFlag) {
        uart_println("[DEBUG] Empty text vibration sequence complete");
